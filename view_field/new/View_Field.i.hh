 ...
 */
template<typename T>
View_Field<T>::View_Field(pointer      begin,
                          pointer      end,
                          unsigned int stride)
    : d_begin_iterator(begin, stride)
    , d_end_iterator(end, stride)
{
 ...
 */
template<typename T>
View_Field<T>::View_Field(iterator begin,
                          iterator end)
...
View_Field<T>::View_Field(Container&   c,
                          unsigned int stride)
    : d_begin_iterator(&(*std::begin(c)), stride)
    , d_end_iterator(&(*std::end(c)), stride)
{
    Ensure(std::distance(d_begin_iterator, d_end_iterator) >= 0);
}
 ...
 */
template<typename T>
const_View_Field<T>::const_View_Field(const_pointer begin,
                                      const_pointer end,
                                      unsigned int  stride)
    : d_begin_iterator(begin, stride)
    , d_end_iterator(end, stride)
 ...
 */
template<typename T>
const_View_Field<T>::const_View_Field(const_iterator begin,
                                      const_iterator end)
    : d_begin_iterator(begin)
    , d_end_iterator(end)
{
...
{
    Require(stride >= 1);
    Require(std::distance(std::begin(c), std::end(c)) ==
...
{
    Require(d_begin_iterator.is_valid());
    Require(d_end_iterator.is_valid());
