...
class Universe
{
  ...
  public:
    ...
    virtual void move_within_cell(double distance, GG_State& state) const = 0;
    ...
    virtual void entered_universe(Sense       local_sense,
                                  surfid_type local_surface,
                                  GG_State&   state) const = 0;
};

class Dummy_Universe : public Universe
{
    //! Move to and logically across the next surface
    virtual void move_across_surface(GG_State& state) const override;

    //! Move along the current direction, not through any surfaces
    virtual void move_within_cell(double distance, GG_State& state) const override;

    //! We just entered a new universe across some unknown bounding surface
    virtual void entered_universe(GG_State& state) const override;

    //! We just entered a new universe (coordinates are local)
    virtual void entered_universe(Sense       local_sense,
                                  surfid_type local_surface,
                                  GG_State&   state) const override;
};

//! Move along the current direction, not through any surfaces
void Dummy_Universe::move_within_cell(double distance, GG_State& state) const
{
    state.x += distance;
}
...
//! We just entered a new universe (coordinates are local)
void Dummy_Universe::entered_universe(Sense       local_sense,
                              surfid_type local_surface,
                              GG_State&   state) const
{
