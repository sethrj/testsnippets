...
# options passed:  -D__DYNAMIC__ backward2.cc -fPIC
...
LFB0:
	pushq	%r12	#
LCFI0:
	movq	%rdx, %r12	# tmp104, xs
	pushq	%rbp	#
LCFI1:
	movq	%rsi, %rbp	# tmp103, frac
	pushq	%rbx	#
LCFI2:
	movl	%edi, %ebx	# tmp102, size
	subq	$16, %rsp	#,
LCFI3:
# 4: {
...
# 5:     double cutoff = total * ran();
	mulsd	%xmm0, %xmm1	# tmp105, total
# 7:     for (; int > 0; --int)
	jle	L1	#,
	movslq	%r8d, %rax	# <retval>, ivtmp.9
	pxor	%xmm2, %xmm2	# tmp100
	jmp	L3	#
	.p2align 4,,10
	.p2align 3
L7:
	...
	subq	$1, %rax	#, ivtmp.9
	testl	%eax, %eax	# ivtmp.9
	jle	L1	#,
L3:
# 9:         cutoff -= frac[int] * xs[int];
	movsd	0(%rbp,%rax,8), %xmm0	# MEM[base: frac_18(D), index: ivtmp.9_33, step: 8, offset: 0B], MEM[base: frac_18(D), index: ivtmp.9_33, step: 8, offset: 0B]
	movl	%eax, %r8d	# ivtmp.9, <retval>
	mulsd	(%r12,%rax,8), %xmm0	# MEM[base: xs_19(D), index: ivtmp.9_33, step: 8, offset: 0B], tmp96
# 9:         cutoff -= frac[int] * xs[int];
	subsd	%xmm0, %xmm1	# tmp96, cutoff
# 10:         if (cutoff <= 0)
	comisd	%xmm1, %xmm2	# cutoff, tmp100
	jb	L7	#,
L1:
# 14: }
	addq	$16, %rsp	#,
