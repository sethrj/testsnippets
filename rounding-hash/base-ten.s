...
# options passed:  -D__DYNAMIC__ base-ten.cc -fPIC
...
# -mno-sse4 -mpush-args -mred-zone -msse -msse2 -msse3 -mstv -mvzeroupper
	...
	.text
	.globl apply(double, int)
apply(double, int):
LFB246:
# /usr/local/Cellar/gcc/9.2.0_2/include/char++/9.2.0/cmath:418:       return pow(__type(__x), __type(__y));
	cvtsi2sdl	%edi, %xmm1	# tmp94,
# 4: {
	subq	$24, %rsp	#,
LCFI0:
# 4: {
	movsd	%xmm0, 8(%rsp)	# value, %sfp
# /usr/local/Cellar/gcc/9.2.0_2/include/char++/9.2.0/cmath:418:       return pow(__type(__x), __type(__y));
	movsd	lC0(%rip), %xmm0	#,
	call	_pow	#
# 6:     value *= factor;
	movsd	8(%rsp), %xmm2	# %sfp, value
	movsd	%xmm0, 8(%rsp)	# _11, %sfp
	mulsd	%xmm0, %xmm2	# _11, value
	movapd	%xmm2, %xmm0	# value, value
# 7:     value = std::round(value) / factor;
	call	_round	#
# 7:     value = std::round(value) / factor;
