...
# options passed:  -D__DYNAMIC__ base-two.cc -fPIC
...
LFB246:
	pushq	%rbx	#
LCFI0:
	movl	%edi, %ebx	# tmp94, digits
	subq	$16, %rsp	#,
LCFI1:
# 8:     value      = std::frexp(value, &exp);
	leaq	12(%rsp), %rdi	#, tmp90
# 7:     int exp    = 0;
	movl	$0, 12(%rsp)	#, exp
# 8:     value      = std::frexp(value, &exp);
	call	_frexp	#
# 10:     value = std::ldexp(value, digits);
	movl	%ebx, %edi	# digits,
	call	_ldexp	#
# 12:     value = std::round(value);
	call	_round	#
# 15:     value = std::ldexp(value, exp - digits);
	movl	12(%rsp), %edi	# exp, tmp91
# 17: }
	addq	$16, %rsp	#,
LCFI2:
# 15:     value = std::ldexp(value, exp - digits);
	subl	%ebx, %edi	# digits, tmp91
# 17: }
	popq	%rbx	#
LCFI3:
# 15:     value = std::ldexp(value, exp - digits);
