...
# 5: {
	...
	leaq	8(%rsp), %rbx	#, ivtmp.11
	leaq	16(%rsp), %rbp	#, _16
	.p2align 4,,10
	.p2align 3
L2:
# 9:         f(bytes[i]);
	movzbl	(%rbx), %edi	# MEM[(const unsigned char *)_14], MEM[(const unsigned char *)_14]
# 7:     for (std::size_t i = 0; i < sizeof(std::size_t); ++i)
	addq	$1, %rbx	#, ivtmp.11
# 9:         f(bytes[i]);
	call	f(unsigned char)	#
# 7:     for (std::size_t i = 0; i < sizeof(std::size_t); ++i)
	cmpq	%rbp, %rbx	# _16, ivtmp.11
	jne	L2	#,
# 11: }
	addq	$24, %rsp	#,
LCFI3:
...
# 19:         f(bytes[i]);
	movzbl	(%rbx), %edi	# MEM[(const unsigned char *)_14], MEM[(const unsigned char *)_14]
# 17:     for (std::size_t i = 0; i < sizeof(std::size_t); ++i)
	addq	$1, %rbx	#, ivtmp.22
# 19:         f(bytes[i]);
	call	f(unsigned char)	#
# 17:     for (std::size_t i = 0; i < sizeof(std::size_t); ++i)
	cmpq	%rbp, %rbx	# _16, ivtmp.22
	jne	L7	#,
# 21: }
	addq	$24, %rsp	#,
LCFI9:
...
LCFI12:
# 24: {
	movq	%rdi, %rbx	# tmp125, value
# 29:         f(bytes[i]);
	movzbl	%dil, %edi	# value, value
	call	f(unsigned char)	#
	movzbl	%bh, %edi	# value, tmp101
	call	f(unsigned char)	#
# 29:         f(bytes[i]);
	movq	%rbx, %rdi	# value, tmp103
	shrq	$16, %rdi	#, tmp103
# 29:         f(bytes[i]);
	movzbl	%dil, %edi	# tmp103, tmp105
	call	f(unsigned char)	#
	movl	%ebx, %edi	# value, tmp109
	shrl	$24, %edi	#, tmp109
	call	f(unsigned char)	#
# 29:         f(bytes[i]);
	movq	%rbx, %rdi	# value, tmp111
	shrq	$32, %rdi	#, tmp111
# 29:         f(bytes[i]);
	movzbl	%dil, %edi	# tmp111, tmp113
	call	f(unsigned char)	#
# 29:         f(bytes[i]);
	movq	%rbx, %rdi	# value, tmp115
	shrq	$40, %rdi	#, tmp115
# 29:         f(bytes[i]);
	movzbl	%dil, %edi	# tmp115, tmp117
	call	f(unsigned char)	#
# 29:         f(bytes[i]);
	movq	%rbx, %rdi	# value, tmp119
# 29:         f(bytes[i]);
	shrq	$56, %rbx	#, value
# 29:         f(bytes[i]);
	shrq	$48, %rdi	#, tmp119
# 29:         f(bytes[i]);
	movzbl	%dil, %edi	# tmp119, tmp121
	call	f(unsigned char)	#
	movq	%rbx, %rdi	# value, tmp124
# 31: }
	popq	%rbx	#
LCFI13:
# 29:         f(bytes[i]);
