...
# options passed:  -D__DYNAMIC__ uninlined-objects.cc -fPIC
...
# 36:     switch (short)
	...
	testb	%dil, %dil	# short
	je	L9	#,
# 35:     double distance = -1;
	movsd	lC0(%rip), %xmm0	#, <retval>
# 44: }
	ret
	.align 4,0x90
L9:
# 18:         return (d_position - pos[N]) / dir[N];
	movsd	(%rsi), %xmm0	# *coeff_ptr_7(D), *coeff_ptr_7(D)
	subsd	(%rdx), %xmm0	# *pos_8(D), tmp105
	divsd	(%rcx), %xmm0	# *dir_9(D), <retval>
# 38:         case (Surface_Type::PX): {distance = Ortho_Plane<0>(*coeff_ptr).calc_intersection(pos, dir);}; break;
	ret
	.align 4,0x90
L4:
# 18:         return (d_position - pos[N]) / dir[N];
...
# 40:         case (Surface_Type::PZ): {distance = Ortho_Plane<2>(*coeff_ptr).calc_intersection(pos, dir);}; break;
	ret
	.align 4,0x90
L3:
# 18:         return (d_position - pos[N]) / dir[N];
...
# 39:         case (Surface_Type::PY): {distance = Ortho_Plane<1>(*coeff_ptr).calc_intersection(pos, dir);}; break;
	ret
LFE2:
	.literal8
	.align 3
